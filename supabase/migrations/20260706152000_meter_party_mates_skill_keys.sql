-- Party mate tooltips: prefer processed leaderboard identity on the ranked parse,
-- expose parse skill keys for client alternate-structure resolution, and mate role bucket.

drop function if exists public.get_meter_leaderboard_party_mates(text, int, timestamptz, timestamptz);

create or replace function public.get_meter_leaderboard_party_mates(
  p_dungeon_id text,
  p_difficulty_id int,
  p_window_start timestamptz default null,
  p_window_end timestamptz default null
)
returns table (
  role_bucket text,
  player_key text,
  mate_player_key text,
  mate_display_name text,
  digimon_id text,
  digimon_name text,
  icon_id text,
  portrait_url text,
  skill_keys jsonb,
  mate_role_bucket text,
  mate_order int,
  parse_duration_sec numeric
)
language sql
stable
security definer
set search_path = public
as $$
  with best as (
    select distinct on (e.role_bucket, lower(trim(e.player_key)))
      e.role_bucket,
      lower(trim(e.player_key)) as player_key,
      e.parse_id,
      coalesce(
        nullif((p.payload->'dungeon'->'clientComplete'->>'timeSec')::numeric, 0),
        coalesce(p.duration_sec, 0),
        coalesce((p.payload->>'sessionDurationSec')::numeric, 0),
        coalesce((
          select max(coalesce((mem.elem->>'durationSec')::numeric, 0))
          from jsonb_array_elements(coalesce(p.payload->'members', '[]'::jsonb)) mem(elem)
        ), 0)
      )::numeric as parse_duration_sec
    from public.meter_leaderboard_entries e
    join public.meter_parses p on p.id = e.parse_id
    where e.dungeon_id = trim(p_dungeon_id)
      and e.difficulty_id = p_difficulty_id
      and e.dps > 0
      and e.role_bucket is not null
      and e.player_key is not null
      and trim(e.player_key) <> ''
      and (p_window_start is null or e.created_at >= p_window_start)
      and (p_window_end is null or e.created_at < p_window_end)
    order by e.role_bucket, lower(trim(e.player_key)), e.dps desc, e.created_at desc
  ),
  parse_members as (
    select
      b.role_bucket,
      b.player_key,
      b.parse_id,
      b.parse_duration_sec,
      m.member,
      public.member_player_key_from_payload(m.member) as member_key
    from best b
    join public.meter_parses p on p.id = b.parse_id
    cross join lateral jsonb_array_elements(coalesce(p.payload->'members', '[]'::jsonb)) as m(member)
  ),
  mates as (
    select
      pm.role_bucket,
      pm.player_key,
      pm.parse_id,
      pm.parse_duration_sec,
      pm.member,
      pm.member_key as mate_player_key,
      coalesce(
        nullif(trim(pm.member->>'tamerName'), ''),
        nullif(trim(pm.member->>'displayLabel'), ''),
        pm.member_key
      ) as mate_display_name,
      row_number() over (
        partition by pm.role_bucket, pm.player_key
        order by coalesce((pm.member->>'totalDamage')::numeric, 0) desc, pm.member_key
      )::int as mate_order
    from parse_members pm
    where pm.member_key <> ''
      and pm.member_key <> pm.player_key
  )
  select
    m.role_bucket,
    m.player_key,
    m.mate_player_key,
    m.mate_display_name,
    coalesce(
      nullif(trim(me.digimon_id), ''),
      nullif(trim(d.primary->>'digimonId'), ''),
      ''
    ) as digimon_id,
    coalesce(
      nullif(trim(me.digimon_name), ''),
      nullif(trim(d.primary->>'digimonName'), ''),
      ''
    ) as digimon_name,
    nullif(
      trim(
        coalesce(
          me.icon_id,
          d.primary->>'iconId',
          d.primary->>'portraitIconId'
        )
      ),
      ''
    ) as icon_id,
    nullif(
      trim(coalesce(me.portrait_url, d.primary->>'portraitUrl')),
      ''
    ) as portrait_url,
    coalesce(sk.keys, '[]'::jsonb) as skill_keys,
    nullif(trim(me.role_bucket), '') as mate_role_bucket,
    m.mate_order,
    m.parse_duration_sec
  from mates m
  cross join lateral (
    select public.member_primary_digimon_from_payload(m.member) as primary
  ) d
  cross join lateral (
    select coalesce(
      jsonb_agg(distinct skill_key order by skill_key)
        filter (where skill_key is not null and skill_key <> '' and skill_key <> '(basic)'),
      '[]'::jsonb
    ) as keys
    from (
      select nullif(trim(skill.elem->>'skillKey'), '') as skill_key
      from jsonb_array_elements(coalesce(m.member->'digimons', '[]'::jsonb)) dg(elem)
      cross join lateral jsonb_array_elements(coalesce(dg.elem->'skills', '[]'::jsonb)) skill(elem)
      where coalesce(nullif(trim(dg.elem->>'digimonId'), ''), '') = coalesce(nullif(trim(d.primary->>'digimonId'), ''), '')
    ) keyed
  ) sk
  left join public.meter_leaderboard_entries me
    on me.parse_id = m.parse_id
    and lower(trim(me.player_key)) = m.mate_player_key
    and me.dps > 0
  where coalesce(
    nullif(trim(me.digimon_id), ''),
    nullif(trim(d.primary->>'digimonId'), ''),
    ''
  ) <> ''
    and coalesce(
      nullif(trim(me.digimon_id), ''),
      nullif(trim(d.primary->>'digimonId'), ''),
      ''
    ) <> 'unknown'
  order by m.role_bucket, m.player_key, m.mate_order;
$$;

grant execute on function public.get_meter_leaderboard_party_mates(text, int, timestamptz, timestamptz) to anon, authenticated;
