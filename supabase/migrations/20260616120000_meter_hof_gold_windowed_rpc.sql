-- Cycle-scoped Hall of Fame: compute gold breaks in a time window on the server and return
-- only gold rows (small egress). All-time reads still use meter_hof_gold_entries.

drop function if exists public.get_meter_hof_gold_breaks(text, int);
...
drop function if exists public.get_meter_player_hof_gold_breaks(text, int);
