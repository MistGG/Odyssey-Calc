...
export function hardMeterDungeons(
  dungeons: WikiDungeonListItem[],
): { dungeonId: string; dungeonName: string }[] {
  return dungeonSelectOptions(dungeons).filter(({ dungeonId }) =>
    difficultySelectOptions(dungeons, dungeonId).some((d) => d.difficultyId === HARD_DIFFICULTY_ID),
  )
}
...
export type MeterEarnMilestoneProgress = {
  ...
  points: number
  granted: boolean
  /** 0/1 = still earnable, 1/1 = already awarded */
  ratio: '0/1' | '1/1'
}
...
export type MeterDungeonEarnProgress = {
  dungeonId: string
  dungeonName: string
  /** Best parse score vs pool for this Hard dungeon (null if no eligible self parse). */
...
/** Leaderboard tier colors for score milestone chips (90 / 99 / 100). */
