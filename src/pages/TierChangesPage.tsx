...
function runKindLabel(row: TierListChangeHistoryRow): string {
  if (row.runKind === 'api_sync') return 'API sync'
  if (row.runKind === 'tier_recompute') return 'Tier recompute'
  if (row.mode === 'force') return 'Full refresh'
  return 'Tier update'
}
