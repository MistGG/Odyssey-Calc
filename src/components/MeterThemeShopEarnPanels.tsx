...
function MilestoneChip({
  ...
  const openTierStyle =
    !granted && tierColor
      ? ({ borderColor: tierColor, color: tierColor } as const)
      : undefined
    ...
    <div className="meter-shop-earn-panels">
      <section className="meter-shop-earn-panel meter-shop-earn-panel--onetime">
        <div className="meter-shop-onetime-head">
          ...
          >
            {oneTimeOpen ? 'Hide rewards' : 'Show rewards'}
          </button>
        </div>
        <p className="meter-parses-muted meter-shop-onetime-sub">
          Per Hard dungeon · each milestone once
        </p>
        <OneTimePointsLegend />
        {oneTimeOpen ? (
          <div className="meter-shop-onetime-body">
            {loading ? (
              <p className="meter-parses-muted">Loading dungeons…</p>
            ) : dungeonProgress.length === 0 ? (
              <p className="meter-parses-muted">No Hard dungeons found in the wiki list.</p>
            ) : (
