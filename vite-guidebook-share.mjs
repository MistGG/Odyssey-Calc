 ...
 * Meter profiles: public/share/meter-player/{key}.html (CI sync) or Supabase public storage
 */
import fs from 'node:fs'
import path from 'node:path'

const GUIDEBOOK_PATH_RE = /^\/(?:Odyssey-Calc\/)?share\/guidebook\/([^/]+)\/?$/
const METER_PLAYER_HTML_RE = /^\/(?:Odyssey-Calc\/)?share\/meter-player\/([^/]+)\.html$/
const METER_PLAYER_OG_RE = /^\/(?:Odyssey-Calc\/)?share\/meter-player\/([^/]+)-og\.png$/
